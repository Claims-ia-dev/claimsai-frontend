...
const DUMMY_CLAIMS = [ // data that the claim has
  {
    id: 'e1',
    customername: 'karla',
...
const UpdateCustomer = () => {
  ...
  useEffect(() => {
    ...
    if (identifiedClaim) { //it uses the claim id so it get the data inside the estimates table , customer info will be in the same table as claim (estimates table in backend)
      setFormData(
        {
          customername: {
            value: '',
  ...
  const claimUpdateSubmitHandler = event => { //handler to send data to update claim and connect to database from there
